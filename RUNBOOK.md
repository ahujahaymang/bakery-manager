...
alters existing ones. When a new column or table is added to a model, run the
migration script on the server:

```bash
aws ssm send-command \
  --instance-ids i-0ff48b32abce4a930 \
  --document-name "AWS-RunShellScript" \
  --parameters 'commands=["python3.11 /opt/kitchenos/scripts/migrate_sqlite.py && systemctl restart kitchenos"]' \
  --region us-east-1
```

The script (`scripts/migrate_sqlite.py`) handles all known schema changes.
When you add a new column to a model, also add it to that script so it runs
